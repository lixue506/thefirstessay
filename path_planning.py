...
# 数据归一化
def sigmoid(x,y):
    s = 1 / (1 + np.exp(-float(x)))
    e = 1 / (1 + np.exp(-float(y)))
    return s,e
# 数据处理
def Get_data(dataset_path):
    ...
    with open('data.csv', 'r',encoding='utf-8') as file:
        ...
        for row in csv_reader:
            ...
            time_live.append(j)
            time_live.append(w)
            dataset.append(time_live)

    return dataset

def main():
    # w_x = np.arange(0, 7.5, 0.05)
    # w_y = np.arange(0, 3, 0.02)
    w_x = [0, 3.5, 7.0]
    w_y = [0, 0.25, 2.0]

    # obstacles
    ob = Get_data('C://Users//1//Desktop//thefirstessay//日期为序的活动轨迹.xlsx')
    ob = np.array(ob)
    ...
    for i in range(500):
        ...
        if show_animation:
            ...
            plt.title("The recommended path")
            plt.grid(True)
            plt.pause(0.0001)
    print('Done')
    if show_animation:
